...
Two-phase approach (DRY RUN - saves GitHub minutes):
  Phase 1 (3:30 AM IST): Select strikes, save entry to active_trade.json
  Phase 2 (5:15 PM IST): Fetch exit prices, calculate P&L, write tracker

Live mode (DRY_RUN=False): Continuous monitoring on Saturdays only.
"""
...
def append_to_tracker(trade_result):
    ...
    # P&L USD (col 17)
    c = ws.cell(row=new_row, column=17)
    c.font = green_font if is_profit else red_font
    c.fill = green_fill if is_profit else red_fill
    c.number_format = '$#,##0.0000;-$#,##0.0000'

    # P&L INR (col 18)
    c = ws.cell(row=new_row, column=18)
    c.font = green_font if is_profit else red_font
    c.fill = green_fill if is_profit else red_fill
    c.number_format = '\u20b9#,##0.00;-\u20b9#,##0.00'

    # P&L % (col 19)
    c = ws.cell(row=new_row, column=19)
    c.font = green_font if is_profit else red_font
    c.fill = green_fill if is_profit else red_fill
...
# =====================================================================
# LIVE MODE: CONTINUOUS MONITORING (only when real money is at stake)
# =====================================================================

def monitor_positions_live(f, entry_call_symbol, entry_put_symbol, entry_call_bid, entry_put_bid,
    ...
    log_print("\n" + "=" * 100, f)
    log_print("LIVE POSITION MONITORING STARTED", f)
    log_print("=" * 100, f)
    log_print(f"Entry CE: ${entry_call_bid:.2f} | PE: ${entry_put_bid:.2f} | Combined: ${entry_combined_premium:.2f}", f)
    log_print(f"SL1: {SL_COMBINED_MULTIPLIER}x >= ${entry_combined_premium * SL_COMBINED_MULTIPLIER:.2f} | SL2: Rs.{HARD_MAX_LOSS_INR:,} | Early: < ${EARLY_EXIT_PREMIUM:.0f} | Time: {EXIT_HOUR}:{EXIT_MINUTE:02d}", f)
    log_print("=" * 100 + "\n", f)
    ...
    while True:
        try:
            ...
            if not call_data['success'] or not put_data['success']:
                log_print(f"[{time_str}] [WARN] Price fetch failed, retrying...", f)
            ...
            log_print(f"[{time_str}] CE: ${cur_ce:.2f} | PE: ${cur_pe:.2f} | Combined: ${cur_combined:.2f} | P&L: ${pnl:+.4f} (Rs.{pnl_inr:+,.2f})", f)

            if cur_combined >= (entry_combined_premium * SL_COMBINED_MULTIPLIER):
                log_print(f"\n[{time_str}] SL1 HIT: Combined {SL_COMBINED_MULTIPLIER}x", f)
                exit_data.update({'exit_ce': cur_ce, 'exit_pe': cur_pe, 'exit_combined': cur_combined, 'exit_reason': f'SL - Combined {SL_COMBINED_MULTIPLIER}x', 'exit_time': time_str})
            ...
            if loss_inr >= HARD_MAX_LOSS_INR:
                log_print(f"\n[{time_str}] HARD CAP HIT: Rs.{loss_inr:,.0f}", f)
                exit_data.update({'exit_ce': cur_ce, 'exit_pe': cur_pe, 'exit_combined': cur_combined, 'exit_reason': f'Hard Cap Rs.{HARD_MAX_LOSS_INR:,}', 'exit_time': time_str})
                _close_both_legs(f, call_product_id, put_product_id, f"Hard Cap Rs.{HARD_MAX_LOSS_INR:,}")
                break

            if cur_combined < EARLY_EXIT_PREMIUM:
                log_print(f"\n[{time_str}] EARLY EXIT: Premium ${cur_combined:.2f}", f)
                exit_data.update({'exit_ce': cur_ce, 'exit_pe': cur_pe, 'exit_combined': cur_combined, 'exit_reason': 'Early Exit - Premium decayed', 'exit_time': time_str})
            ...
            exit_data.update({'exit_reason': 'Interrupted', 'exit_time': datetime.now(IST).strftime('%H:%M:%S')})
            break
        except Exception as e:
...
with open(log_file, 'w', encoding='utf-8') as f:
    try:
        ...
        log_print("=" * 120, f)
        log_print(f"  BTC SHORT STRANGLE - {'DRY RUN' if DRY_RUN else 'LIVE'} - {today.strftime('%A')} - Phase: {phase}", f)
        log_print("=" * 120, f)
        log_print(f"Timestamp: {today.strftime('%d-%m-%Y %H:%M:%S IST')}", f)
        log_print(f"Position: {POSITION_SIZE_LOTS} lots/leg ({POSITION_SIZE_BTC} BTC) | USD/INR: {usd_to_inr:.2f}", f)
        log_print("=" * 120 + "\n", f)

        # ═══════════════════════════════════════════════════════════
        # PHASE: ENTRY (3:30 AM) - Select strikes, save entry data
        # ═══════════════════════════════════════════════════════════
        ...
        if phase == "ENTRY":
            ...
            if best_combo:
                ...
                if DRY_RUN:
                    # ─── DRY RUN: Save entry data and exit immediately ───
                    log_print("[DRY RUN] Saving entry data for exit phase at 5:15 PM...\n", f)
                    ...
                    log_print(f"[DRY RUN] Entry saved to {ACTIVE_TRADE_FILE}", f)
                    log_print(f"[DRY RUN] Exit phase will run at 5:15 PM IST to capture exit prices.\n", f)

                elif is_saturday:
                    # ─── LIVE: Place orders + continuous monitoring ───
                    ...
                    if available_balance < total_margin:
                        ...
                        exit_data = monitor_positions_live(
                            f=f,
                        ...
                        try:
                            append_to_tracker({
                                'date': today.strftime('%d-%m-%Y'), 'day': today.strftime('%A'),
                                'entry_time': entry_time_str, 'exit_time': exit_data.get('exit_time', ''),
                                ...
                                'entry_combined': combined_premium,
                                'exit_ce': exit_data.get('exit_ce', 0), 'exit_pe': exit_data.get('exit_pe', 0),
                                'exit_combined': exit_combined,
                                'pnl_usd': pnl_usd, 'pnl_inr': pnl_inr,
                                'exit_reason': exit_data.get('exit_reason', ''),
                                'duration': duration_str, 'mode': 'LIVE'
                            })
                        except Exception as e:
                            log_print(f"[ERROR] Tracker: {e}\n", f)
                else:
                    log_print(f"[INFO] {today.strftime('%A')} - Live orders only on Saturday.\n", f)
            else:
                log_print("[INFO] No valid strikes found. Skipping today.\n", f)

            # ─── OPTION CHAIN (always display) ───
            log_print("=" * 160, f)
        ...
        # ═══════════════════════════════════════════════════════════
        # PHASE: EXIT (5:15 PM) - Fetch exit prices, write tracker
        # ═══════════════════════════════════════════════════════════

        elif phase == "EXIT":
            log_print("[EXIT PHASE] Loading entry data...\n", f)
            ...
            # Fetch current exit prices
            log_print("Fetching exit prices...\n", f)
            call_data = get_current_premium(entry['call_symbol'])
            put_data = get_current_premium(entry['put_symbol'])

            if not call_data['success'] or not put_data['success']:
                log_print("[ERROR] Failed to fetch exit prices. Retrying once...", f)
                time.sleep(5)
                call_data = get_current_premium(entry['call_symbol'])
                put_data = get_current_premium(entry['put_symbol'])

            exit_ce = call_data['ask'] if call_data['success'] else 0
            exit_pe = put_data['ask'] if put_data['success'] else 0
            exit_combined = exit_ce + exit_pe

            entry_combined = entry['entry_combined']
            pnl_usd = (entry_combined - exit_combined) * POSITION_SIZE_BTC
            pnl_inr = pnl_usd * entry.get('usd_to_inr', usd_to_inr)

            # Determine what exit reason would have been
            if exit_combined >= (entry_combined * SL_COMBINED_MULTIPLIER):
                exit_reason = f"SL - Combined {SL_COMBINED_MULTIPLIER}x"
            elif (exit_combined - entry_combined) * POSITION_SIZE_BTC * entry.get('usd_to_inr', usd_to_inr) >= HARD_MAX_LOSS_INR:
            ...
            exit_time_str = today.strftime('%H:%M')

            # Calculate duration
            try:
                entry_parts = entry['entry_time'].split(':')
                entry_dt = today.replace(hour=int(entry_parts[0]), minute=int(entry_parts[1]), second=0)
                dur_s = int((today - entry_dt).total_seconds())
                if dur_s < 0:
                    dur_s = 0
                duration_str = f"{dur_s // 3600}h {(dur_s % 3600) // 60}m"
            except:
            ...
            log_print("=" * 120, f)
            log_print(f"Exit CE: ${exit_ce:.2f} | Exit PE: ${exit_pe:.2f} | Exit Combined: ${exit_combined:.2f}", f)
            log_print(f"P&L: ${pnl_usd:+.4f} (Rs.{pnl_inr:+,.2f})", f)
            log_print(f"Exit Reason: {exit_reason}", f)
            log_print(f"Duration: {duration_str}", f)
            log_print("=" * 120 + "\n", f)
            ...
            try:
                append_to_tracker({
                    ...
                    'pnl_usd': pnl_usd, 'pnl_inr': pnl_inr,
                    'exit_reason': exit_reason, 'duration': duration_str,
            ...
            # Clean up active trade file
            os.remove(ACTIVE_TRADE_FILE)
            log_print(f"[CLEANUP] Removed {ACTIVE_TRADE_FILE}\n", f)

        # ═══════════════════════════════════════════════════════════
        # PHASE: NO_TRADE - Exit run called but no active trade
        # ═══════════════════════════════════════════════════════════

        elif phase == "NO_TRADE":
            log_print("[INFO] No active trade to exit. Nothing to do.\n", f)
